...
class WorldStateTest {
    ...
    @Test
    void testNewWorldGeneratedWhenAllCoinsCollected() {
        List<Coordinate> allCoins = new ArrayList<>(worldState.getCoins());


        // Collect all coins manually
        for (Coordinate coin : allCoins) {
            worldState.updatePositin(coin.x, coin.y);
        }

        int oldSeed = worldState.getSeed();
        worldState.updateState(); // should trigger new world

        assertThat(worldState.getSeed()).isGreaterThan(oldSeed);
        assertThat(worldState.getCoins().size()).isGreaterThan(0); // new coins
    }
}
